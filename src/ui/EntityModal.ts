          ...
          if (propertyType === 'Checkbox') {
            ...
            const searchCitations = async () => {
              ...
              try {
                ...
                // 使用Obsidian的search API搜索笔记
                // 注意：Obsidian的Vault API中没有直接的search方法，我们需要遍历所有文件并手动过滤
                ...
                const filteredFiles = allFiles.filter(file => {
                  ...
                  // 对于markdown文件，我们需要读取内容并检查front matter
                  if (file.extension === 'md') {
                    // 这里我们只检查文件名，因为读取大量文件内容会影响性能
                    // 如果需要更精确的搜索，可以异步读取文件内容
                    return true;
                  }
